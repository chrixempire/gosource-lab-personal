...
const emit = defineEmits<{
  apply: [next: Partial<CustomerOrderHistoryFilters>];
  clearAll: [];
}>();
...
<template>
  <div class="flex flex-wrap items-center gap-2">
    ...
    <OrderFilterPopover
      ...
      :active="filters.paymentStatus.length > 0"
      @update:open="(value) => value && syncDrafts()"
      @apply="emit('apply', { paymentStatus: [...draftPaymentStatus] })"
      @clear="emit('apply', { paymentStatus: [] })"
    >
      <div class="flex flex-col gap-2">
        ...
        >
          <Checkbox
            ...
            @update:model-value="draftPaymentStatus = toggle(draftPaymentStatus, option.value)"
          />
