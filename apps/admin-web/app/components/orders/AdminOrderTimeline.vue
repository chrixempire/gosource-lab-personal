...
import type { AdminOrderTimelineEvent } from '~/lib/order-details';
...
<template>
  <section class="h-full rounded-[20px] border border-grey-50 bg-white p-5">
    <h2 class="text-base font-semibold text-grey-900">Order timeline</h2>

    <div v-if="loading" class="mt-5 space-y-4" aria-busy="true">
    ...
    <ol v-else class="mt-5 space-y-0">
      ...
      >
        ...
        <div class="min-w-0 flex-1 pb-5">
          <p class="text-sm font-semibold text-grey-900">{{ event.title }}</p>
          <p v-if="event.description" class="mt-1 text-sm text-grey-600">
            {{ event.description }}
          </p>
          <p class="mt-2 text-xs text-grey-300">{{ event.updatedLabel }}</p>
        </div>
      </li>
    </ol>
  </section>
