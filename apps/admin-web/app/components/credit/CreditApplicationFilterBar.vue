...
import {
  CREDIT_APPLICATION_STATUS_OPTIONS,
  CREDIT_APPLICATION_TYPE_OPTIONS,
} from '~/lib/credit-constants';
import type { CreditApplicationListFilters } from '~/types/credit';

const props = defineProps<{ filters: CreditApplicationListFilters }>();
const emit = defineEmits<{
  apply: [next: Partial<CreditApplicationListFilters>];
  clearAll: [];
}>();
...
const draftType = ref<CreditApplicationListFilters['applicationType']>([]);
const draftStatus = ref<CreditApplicationListFilters['status']>([]);
const draftStart = ref('');
const draftEnd = ref('');

function sync() {
  draftType.value = [...props.filters.applicationType];
...
const hasActive = computed(
  () =>
    props.filters.applicationType.length > 0 ||
...
<template>
  <div class="flex min-w-0 flex-wrap items-center gap-2">
    <OrderFilterPopover
      ...
      :active="filters.applicationType.length > 0"
      @open="sync"
      @apply="emit('apply', { applicationType: [...draftType], page: 1 })"
      @clear="emit('apply', { applicationType: [], page: 1 })"
    >
      <div class="flex flex-col gap-2">
        <label
          v-for="option in CREDIT_APPLICATION_TYPE_OPTIONS"
          :key="option.value"
    ...
    >
      <div class="flex flex-col gap-2">
        <label
          v-for="option in CREDIT_APPLICATION_STATUS_OPTIONS"
          :key="option.value"
          class="flex cursor-pointer items-center gap-2 text-sm"
        >
          <Checkbox
