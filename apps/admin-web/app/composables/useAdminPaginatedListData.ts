...
import {
  readAdminAuthenticatedAsyncCache,
...
function resolveListCachedData<ResT>(
  ...
  return readAdminAuthenticatedAsyncCache<ResT>(signature);
}
 ...
 */
export async function useAdminPaginatedListData<ResT, DataT = ResT>(
  baseKey: string,
  keyParts: MaybeRefOrGetter<ListKeyPart[]>,
  handler: () => Promise<ResT>,
  options?: AdminAuthenticatedAsyncDataOptions<ResT, DataT, never, undefined> & {
    pageCache?: boolean;
  },
) {
  const { whenReady, session } = useAdminSession();
  const listCacheBaseKey = computed(
    () => `${baseKey}:${getAdminSessionCacheSignature(session.value)}`,
  );
  ...
  const result = await useAdminAuthenticatedAsyncData(baseKey, fetchAndCache, {
    fastNav: true,
    ...rest,
