  ...
  <section class="flex flex-col gap-4">
    ...
    <div v-if="loading" class="grid gap-3 md:hidden">
      <div
        v-for="index in 3"
        :key="index"
        class="rounded-[18px] border border-grey-50 bg-white p-4"
      >
    ...
    <div v-else-if="requests.length" class="grid gap-3 md:hidden">
      <article
        v-for="request in requests"
        :key="request.id"
        class="cursor-pointer rounded-[18px] border border-grey-50 bg-white p-4 transition-colors duration-150 hover:bg-primary-50/45"
        @click="emit('select', request)"
      >
        <div class="flex items-start justify-between gap-3">
