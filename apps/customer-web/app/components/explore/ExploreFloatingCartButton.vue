...
<template>
  <button
    ...
    class="customer-brand-hero fixed z-40 flex max-w-[min(100vw-2rem,18.5rem)] cursor-pointer items-center gap-3 rounded-full py-2 pl-2 pr-3.5 text-left shadow-[0_20px_48px_-16px_rgba(4,85,11,0.55),0_12px_32px_-12px_rgba(16,24,40,0.45)] transition hover:brightness-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400 focus-visible:ring-offset-2 focus-visible:ring-offset-background-on-canvas dark:shadow-[0_20px_48px_-16px_rgba(0,0,0,0.5)] bottom-[max(1.25rem,env(safe-area-inset-bottom))] right-[max(1.25rem,env(safe-area-inset-right))] sm:bottom-6 sm:right-6"
    :aria-label="isAddingToRequest ? 'View request items' : 'Open cart'"
    @click="openCart"
  >
    <span
      class="flex size-10 shrink-0 items-center justify-center rounded-full bg-[#101928]/85 text-[13px] font-bold tabular-nums text-white dark:bg-black/35"
    >
      {{ countLabel }}
    </span>
