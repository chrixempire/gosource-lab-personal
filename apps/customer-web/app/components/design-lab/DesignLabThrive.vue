...
// Soft, wellness category rail (Thrive: cream pills, coral active state).
...
<template>
  <div :style="body" class="rounded-xl bg-[#FAF3EF] p-4 text-[#333333]">
    ...
    <!-- Wellness category rail — Thrive: soft rounded cream chips, coral active pill, hidden scrollbar. -->
    <nav class="mb-4 -mx-1 flex gap-2 overflow-x-auto px-1 py-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      <button
        ...
        :class="activeCat === c
          ...
          : 'bg-[#FBF3EE] text-[#EC6E55] hover:bg-[#F6E6DE]'"
        @click="activeCat = c"
      >{{ c }}</button>
    </nav>

    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      <div v-for="p in visibleProducts" :key="p.id" class="rounded-[5px] border bg-white p-3" :class="cart.qtyOfProduct(p.id) ? 'ring-1 ring-[#EC6E55] border-[#EC6E55]' : 'border-[#D6D6D6]'">
        <span class="inline-block rounded-full bg-[#EEF4F1] px-2 py-0.5 text-[11px] font-semibold text-[#365D5A]">Member Favorite</span>
        <div class="relative flex aspect-square cursor-pointer items-center justify-center overflow-hidden bg-white" @click="openModal(p)">
          <span v-if="designLabDiscount(p)" class="absolute left-1 top-1 z-[1] rounded-full bg-[#EC6E55] px-2 py-0.5 text-[11px] font-bold text-white shadow-sm">{{ designLabDiscount(p)!.pct }}% off</span>
        ...
        <div class="flex items-baseline gap-2">
          ...
          <span v-if="designLabDiscount(p)?.compareNaira" class="text-[12px] text-[#707070] line-through">{{ formatNaira(designLabDiscount(p)!.compareNaira!) }}</span>
          <span v-else-if="p.compareAtNaira" class="text-[12px] text-[#707070] line-through">{{ formatNaira(p.compareAtNaira) }}</span>
        </div>
        <p v-if="designLabDiscount(p)" class="text-[11px] font-medium text-[#365D5A]">Member price</p>
    ...
    <Teleport to="body">
      <div v-if="selected" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 p-4" @click.self="selected = null">
        <div :style="body" class="relative w-full max-w-[420px] rounded-2xl bg-[#FDF9F7] p-6 text-[#333] shadow-2xl">
          ...
          <div class="relative flex aspect-square items-center justify-center overflow-hidden rounded-xl bg-white p-3"><span v-if="designLabDiscount(selected)" class="absolute left-2 top-2 z-[1] rounded-full bg-[#EC6E55] px-2.5 py-1 text-[12px] font-bold text-white shadow-sm">{{ designLabDiscount(selected)!.pct }}% off</span><img v-if="selected.imageUrl" :src="selected.imageUrl" class="h-full w-full object-contain" /><span v-else class="text-6xl">🛒</span></div>
          ...
          <div class="flex items-baseline gap-2">
            ...
            <span v-if="designLabDiscount(selected)?.compareNaira" class="text-[13px] text-[#707070] line-through">{{ formatNaira(designLabDiscount(selected)!.compareNaira!) }}</span>
            <span v-else-if="selected.compareAtNaira" class="text-[13px] text-[#707070] line-through">{{ formatNaira(selected.compareAtNaira) }}</span>
          </div>
          <DesignLabUnitPicker
            v-if="units.length > 1"
            class="mt-4"
      ...
      <div v-if="pdp" :style="body" class="fixed inset-0 z-[200] overflow-y-auto bg-[#FDF9F7] text-[#333]">
        ...
        <div class="mx-auto max-w-[980px] px-4 py-8">
          <div class="grid grid-cols-1 gap-10 md:grid-cols-2">
            <div class="relative flex aspect-square items-center justify-center overflow-hidden rounded-2xl bg-white p-8"><span v-if="designLabDiscount(pdp)" class="absolute left-3 top-3 z-[1] rounded-full bg-[#EC6E55] px-3 py-1 text-[13px] font-bold text-white shadow-sm">{{ designLabDiscount(pdp)!.pct }}% off</span><img v-if="pdp.imageUrl" :src="pdp.imageUrl" class="h-full w-full object-contain" /><span v-else class="text-7xl">🛒</span></div>
            <div>
              ...
              <div class="flex items-baseline gap-2">
                ...
                <span v-if="designLabDiscount(pdp)?.compareNaira" class="text-[15px] text-[#707070] line-through">{{ formatNaira(designLabDiscount(pdp)!.compareNaira!) }}</span>
                <span v-else-if="pdp.compareAtNaira" class="text-[15px] text-[#707070] line-through">{{ formatNaira(pdp.compareAtNaira) }}</span>
              </div>
              <DesignLabUnitPicker
                v-if="units.length > 1"
                class="mt-4"
