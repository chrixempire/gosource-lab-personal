...
// Flink category rail — "All" plus distinct categories; main grid filters to active.
...
<template>
  <div :style="font" class="text-[#161616]">
    <DesignLabCartBar :count="cart.count.value" :total="cart.total.value" :accent="ACCENT" :fg="FG" />

    <!-- Flink category rail: bold-pink active chip, pale-pink rest. Hide scrollbar. -->
    <div class="-mx-1 mb-4 flex gap-2 overflow-x-auto px-1 pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      <button
        type="button"
        class="shrink-0 rounded-full px-4 py-2 text-[13px] font-bold transition"
        :class="activeCat === 'All' ? 'bg-[#FF2882] text-white shadow-sm' : 'bg-[#FFE3EF] text-[#FF2882] hover:bg-[#FFD0E4]'"
        @click="activeCat = 'All'"
      >
        All
      </button>
      <button
        ...
        type="button"
        class="shrink-0 rounded-full px-4 py-2 text-[13px] font-bold transition"
        :class="activeCat === c ? 'bg-[#FF2882] text-white shadow-sm' : 'bg-[#FFE3EF] text-[#FF2882] hover:bg-[#FFD0E4]'"
        @click="activeCat = c"
      >
        {{ c }}
      </button>
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      <div v-for="p in visibleProducts" :key="p.id" class="rounded-[12px] border bg-white p-3 transition" :class="cart.qtyOfProduct(p.id) ? 'border-[#FF2882] ring-1 ring-[#FF2882]' : 'border-[#EBEBEB]'">
        <div class="relative">
          <span v-if="designLabDiscount(p)" class="absolute left-0 top-0 z-10 rounded-br-lg rounded-tl-md bg-[#FF2882] px-2 py-0.5 text-[11px] font-extrabold uppercase tracking-tight text-white shadow-sm">{{ designLabDiscount(p)!.pct }}% OFF</span>
        ...
        <div class="mt-2 flex items-baseline gap-1.5">
          <span class="text-[16px] font-extrabold leading-none" :class="designLabDiscount(p) ? 'text-[#FF2882]' : ''">{{ formatNaira(p.priceNaira) }}</span>
          <span v-if="designLabDiscount(p)?.compareNaira" class="text-[12px] text-[#717171] line-through">{{ formatNaira(designLabDiscount(p)!.compareNaira!) }}</span>
        </div>
        <p class="mt-1 line-clamp-2 cursor-pointer text-[14px] font-medium leading-tight" @click="openModal(p)">{{ p.name }}</p>
        <p class="mt-0.5 text-[12px] text-[#717171]">{{ p.unit }}</p>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="selected" class="fixed inset-0 z-[200] flex items-end justify-center bg-black/50 sm:items-center" @click.self="selected = null">
        <div :style="font" class="relative w-full max-w-[420px] rounded-t-2xl bg-white p-5 text-[#161616] shadow-2xl sm:rounded-2xl">
          ...
          <p class="text-[13px] text-[#717171]">{{ selected.unit }}</p>
          <div class="mt-2 flex items-baseline gap-2">
            <span class="text-[22px] font-extrabold leading-none" :class="designLabDiscount(selected) ? 'text-[#FF2882]' : ''">{{ formatNaira(selPrice) }}</span>
            <span v-if="designLabDiscount(selected)?.compareNaira" class="text-[13px] text-[#717171] line-through">{{ formatNaira(designLabDiscount(selected)!.compareNaira!) }}</span>
            <span v-if="designLabDiscount(selected)" class="rounded-md bg-[#FF2882] px-1.5 py-0.5 text-[11px] font-extrabold text-white">{{ designLabDiscount(selected)!.pct }}% OFF</span>
          </div>
          <DesignLabUnitPicker
            v-if="units.length > 1"
            class="mt-4"
          ...
          <div class="mt-4 flex items-center gap-3">
            <div class="flex items-center gap-3 rounded-full border border-[#EBEBEB] px-2 py-1.5 text-[#FF2882]">
              <button @click="qty = Math.max(1, qty - 1)"><Minus class="size-4" :stroke-width="3" /></button>
              <input type="number" min="1" :value="qty" class="w-9 bg-transparent text-center text-[15px] font-bold text-[#161616] outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none" @change="qty = Math.max(1, Math.round(+($event.target as HTMLInputElement).value || 1))" />
              <button @click="qty++"><Plus class="size-4" :stroke-width="3" /></button>
            </div>
            <button class="flex flex-1 items-center justify-center gap-1.5 rounded-full bg-[#FF2882] py-3 text-[15px] font-bold text-white hover:bg-[#e51f72]" @click="addFromModal"><Check v-if="added" class="size-4" />{{ added ? 'Added to cart' : 'Add to cart' }}</button>
          </div>
          <button class="mt-3 w-full text-center text-[14px] font-bold text-[#FF2882] hover:underline" @click="viewFull">View full page →</button>
        </div>
      </div>
    </Teleport>

    <Teleport to="body">
      <div v-if="pdp" :style="font" class="fixed inset-0 z-[200] overflow-y-auto bg-white text-[#161616]">
        <header class="sticky top-0 z-10 flex items-center border-b border-[#EBEBEB] bg-white px-4 py-3">
          <button class="flex items-center gap-1 text-[14px] font-bold text-[#FF2882]" @click="pdp = null"><ChevronLeft class="size-5" /> Back</button>
        </header>
        <div class="mx-auto max-w-[1000px] px-4 py-6">
          <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
            ...
            <div>
              ...
              <p class="mt-1 text-[14px] text-[#717171]">{{ pdp.unit }}</p>
              <div class="mt-3 flex items-baseline gap-2">
                <span class="text-[30px] font-extrabold leading-none" :class="designLabDiscount(pdp) ? 'text-[#FF2882]' : ''">{{ formatNaira(selPrice) }}</span>
                <span v-if="designLabDiscount(pdp)?.compareNaira" class="text-[15px] text-[#717171] line-through">{{ formatNaira(designLabDiscount(pdp)!.compareNaira!) }}</span>
                <span v-if="designLabDiscount(pdp)" class="rounded-md bg-[#FF2882] px-2 py-0.5 text-[12px] font-extrabold text-white">{{ designLabDiscount(pdp)!.pct }}% OFF</span>
              </div>
              <DesignLabUnitPicker
                v-if="units.length > 1"
                class="mt-4"
              ...
              <div class="mt-5 flex items-center gap-3">
                <div class="flex items-center gap-3 rounded-full border border-[#EBEBEB] px-3 py-2 text-[#FF2882]">
                  <button @click="qty = Math.max(1, qty - 1)"><Minus class="size-5" :stroke-width="3" /></button>
                  <input type="number" min="1" :value="qty" class="w-10 bg-transparent text-center font-bold text-[#161616] outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none" @change="qty = Math.max(1, Math.round(+($event.target as HTMLInputElement).value || 1))" />
                  <button @click="qty++"><Plus class="size-5" :stroke-width="3" /></button>
                </div>
                <button class="flex-1 rounded-full bg-[#FF2882] py-3.5 text-[16px] font-bold text-white hover:bg-[#e51f72]" @click="addFromPdp">Add to cart</button>
              </div>
              <p v-if="cart.qtyOfProduct(pdp.id)" class="mt-2 text-[13px] font-semibold" :style="{ color: ACCENT }">✓ {{ cart.qtyOfProduct(pdp.id) }} in cart</p>
              <div class="mt-6 divide-y divide-[#EBEBEB] border-y border-[#EBEBEB]">
                <details class="py-3" open><summary class="cursor-pointer text-[15px] font-bold">Details</summary><p class="mt-2 text-[14px] text-[#717171]">{{ pdp.description || 'Delivered in minutes by Flink.' }}</p></details>
              </div>
            </div>
          </div>
          <section class="mt-10">
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
              <div v-for="r in related" :key="r.id" class="rounded-[12px] border border-[#EBEBEB] bg-white p-2">
