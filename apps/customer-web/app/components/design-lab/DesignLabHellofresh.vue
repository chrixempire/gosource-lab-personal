...
import { designLabUnits, designLabIsMultiUnit, designLabDiscount, designLabCategories, designLabCategoryEmoji } from '~/lib/design-lab';
...
// Category rail (HelloFresh recipe-collection filter chips).
...
<template>
  <div :style="font" class="text-[#1A1A1A]">
    ...
    <!-- Category rail (HelloFresh lime filter chips). -->
    <div class="hf-rail -mx-1 mb-4 flex gap-2 overflow-x-auto px-1 pb-1">
      <button
        class="shrink-0 rounded-full px-4 py-2 text-[13px] font-bold transition"
        :class="activeCat === 'All' ? 'bg-[#91C813] text-[#1A1A1A]' : 'bg-[#F0F4E6] text-[#3a7d00] hover:bg-[#E4ECCF]'"
        @click="activeCat = 'All'"
      >🍽️ All recipes</button>
      <button
        v-for="c in categories"
        :key="c"
        class="shrink-0 rounded-full px-4 py-2 text-[13px] font-bold transition"
        :class="activeCat === c ? 'bg-[#91C813] text-[#1A1A1A]' : 'bg-[#F0F4E6] text-[#3a7d00] hover:bg-[#E4ECCF]'"
        @click="activeCat = c"
      >{{ designLabCategoryEmoji(c) }} {{ c }}</button>
    </div>

    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <div v-for="p in visibleProducts" :key="p.id" class="overflow-hidden rounded-[14px] border bg-white transition" :class="cart.qtyOfProduct(p.id) ? 'border-[#91C813] ring-1 ring-[#91C813]' : 'border-[#E5E5E5]'">
        <div class="relative cursor-pointer" @click="openModal(p)">
          ...
          <span v-if="designLabDiscount(p)" class="absolute left-3 top-3 rounded-md bg-[#91C813] px-2 py-1 text-[11px] font-extrabold uppercase tracking-wide text-[#1A1A1A]">{{ designLabDiscount(p)!.pct }}% OFF</span>
          <span v-else class="absolute left-3 top-3 rounded-md bg-[#1A1A1A] px-2 py-1 text-[11px] font-bold uppercase tracking-wide text-white">Top rated</span>
        </div>
        <div class="p-4">
          ...
          <div class="mt-2 flex items-center justify-between gap-2">
            <span class="flex items-baseline gap-1.5">
              ...
              <span v-if="designLabDiscount(p)?.compareNaira" class="text-[12px] text-[#9A9A9A] line-through">{{ formatNaira(designLabDiscount(p)!.compareNaira!) }}</span>
            </span>
            <div class="w-[120px]">
    ...
    <Teleport to="body">
      <div v-if="selected" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4" @click.self="selected = null">
        <div :style="font" class="relative w-full max-w-[460px] overflow-hidden rounded-2xl bg-white text-[#1A1A1A] shadow-2xl">
          ...
          <div class="relative flex aspect-[16/10] items-center justify-center overflow-hidden bg-[#F4F4F2]"><img v-if="selected.imageUrl" :src="selected.imageUrl" class="h-full w-full object-cover" /><span v-else class="text-6xl">🍽️</span><span v-if="designLabDiscount(selected)" class="absolute left-3 top-3 rounded-md bg-[#91C813] px-2 py-1 text-[11px] font-extrabold uppercase tracking-wide text-[#1A1A1A]">{{ designLabDiscount(selected)!.pct }}% OFF</span></div>
          <div class="p-5">
            ...
            <p class="mt-2 flex items-baseline gap-2"><span class="text-[22px] font-extrabold">{{ formatNaira(selPrice) }}</span><span v-if="designLabDiscount(selected)?.compareNaira" class="text-[14px] text-[#9A9A9A] line-through">{{ formatNaira(designLabDiscount(selected)!.compareNaira!) }}</span></p>
            <DesignLabUnitPicker
              v-if="units.length > 1"
              class="mt-4"
      ...
      <div v-if="pdp" :style="font" class="fixed inset-0 z-[200] overflow-y-auto bg-white text-[#1A1A1A]">
        ...
        <div class="mx-auto max-w-[1000px] px-4 py-6">
          <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
            ...
            <div>
              <span v-if="designLabDiscount(pdp)" class="rounded-md bg-[#91C813] px-2 py-1 text-[11px] font-extrabold uppercase tracking-wide text-[#1A1A1A]">{{ designLabDiscount(pdp)!.pct }}% OFF</span>
              <span v-else class="rounded-md bg-[#1A1A1A] px-2 py-1 text-[11px] font-bold uppercase tracking-wide text-white">Top rated</span>
              ...
              <p class="mt-3 flex items-baseline gap-2"><span class="text-[30px] font-extrabold leading-none">{{ formatNaira(selPrice) }}</span><span v-if="designLabDiscount(pdp)?.compareNaira" class="text-[16px] text-[#9A9A9A] line-through">{{ formatNaira(designLabDiscount(pdp)!.compareNaira!) }}</span></p>
              <DesignLabUnitPicker
                v-if="units.length > 1"
                class="mt-4"
                :units="units" :selected="selUnit" :accent="ACCENT"
