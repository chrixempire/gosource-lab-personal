...
export default defineEventHandler(async (event) => {
  ...
  const endpoint = `${getCustomerApiBaseUrl(event)}/auth/send-password-email`;
  ...
  try {
    ...
    return forwardApiError(event, error, 'Unable to send password reset email right now') as never;
  }
});
