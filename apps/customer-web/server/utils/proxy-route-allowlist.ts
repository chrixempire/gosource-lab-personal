...
export const PROXY_ROUTE_RULES: ProxyRouteRule[] = [
  ...
  {
    methods: ["PATCH"],
    pattern: /^request\/[^/]+\/(?:approve|reject|cancel)$/,
  },
  { methods: ["GET"], pattern: /^notification$/ },
  ...
  { methods: ["GET"], pattern: /^messaging\/active-alert$/ },
  ...
  {
    methods: ["GET"],
    pattern: /^analytics\/top-procured-items\/branch\/[^/]+$/,
  },
  {
    methods: ["GET"],
    pattern: /^analytics\/total-procurement\/branch\/[^/]+$/,
  },
...
export function getProxyRouteRule(method: string, pathSegments: string[]) {
  ...
  return (
    PROXY_ROUTE_RULES.find(
      (rule) =>
        rule.methods.includes(method) && rule.pattern.test(normalizedPath),
    ) ?? null
  );
}
